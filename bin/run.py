...
def run():
    ...
    with beam.Pipeline(argv=pipeline_args, options=pipeline_options) as p:
        (p
        | beam.io.fileio.MatchFiles(known_args.input) # produces a PCollection of FileMetadata objects
        | beam.io.fileio.ReadMatches()                # produces a PCollection of ReadableFile objects
        | 'Load audio with pytorch' >> beam.ParDo(LoadWithTorchaudio())
        | 'Get writable text' >> beam.Map(lambda x: '{}\t({})'.format(x[0], x[1]))
        | 'Log to local file' >> beam.io.WriteToText(known_args.output, append_trailing_newlines=True)
        )
